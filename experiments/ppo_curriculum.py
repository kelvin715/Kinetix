...
import wandb
from kinetix.environment import LogWrapper, make_reset_fn_from_config
...
def list_grasp_level_paths(root_dir: str, max_stage: int) -> List[str]:
    paths: List[str] = []
    for stage in range(1, max_stage + 1):
        stage_dir = os.path.join(root_dir, f"level_{stage}")
        if not os.path.isdir(stage_dir):
            continue
        for fn in sorted(os.listdir(stage_dir)):
            if fn.endswith(".json"):
                paths.append(os.path.join(stage_dir, fn))
    return paths


def make_train_for_stage(config, env_params, static_env_params, init_train_state: Optional[TrainState]):
    ...
    def train(rng, init_train_state: Optional[TrainState]):
        last_time = time.time()
        # INIT NETWORK
        ...
        if config["load_from_checkpoint"] != None and init_train_state is None:
        ...
        return {"runner_state": runner_state, "metric": metric, "train_state": final_train_state}
...
def main(config):
    config = normalise_config(OmegaConf.to_container(config), "PPO-CURRICULUM")

    # Curriculum options (with sensible defaults)
    curriculum_cfg = config.get("curriculum", {}) if isinstance(config, dict) else {}
    root_dir = curriculum_cfg.get(
        "root_dir",
        "/proj-vertical-llms-pvc/users/zhihan/robots_proj/Kinetix/kinetix/levels/l/grasp_levels",
        # "/proj-vertical-llms-pvc/users/zhihan/robots_proj/Kinetix/kinetix/levels/l/grasp_levels_wo_easy",
    )
    num_stages = int(curriculum_cfg.get("stages", 5))

    # Split total timesteps across stages equally unless overridden
    total_timesteps = int(config["total_timesteps"])
    per_stage_timesteps = int(curriculum_cfg.get("timesteps_per_stage", total_timesteps // num_stages))
    print(f"per_stage_timestep: {per_stage_timesteps / 1e6}M")
    print(f"total_timesteps: {total_timesteps / 1e6}M")
    ...
    if config["use_wandb"]:
        run = init_wandb(config, "PPO-CURRICULUM")

    rng = jax.random.PRNGKey(config["seed"])
    train_state: Optional[TrainState] = None

    for stage in range(1, num_stages + 1):
        levels = list_grasp_level_paths(root_dir, stage)
        if len(levels) == 0:
            raise ValueError(f"No JSON levels found under {root_dir} for stages up to {stage}")

        # Stage-specific config
        stage_cfg = deepcopy(config)
        stage_cfg["train_level_mode"] = "list"
        stage_cfg["train_levels_list"] = levels
        stage_cfg["total_timesteps"] = per_stage_timesteps

        print(f"[Curriculum] Stage {stage}/{num_stages}: {len(levels)} levels, timesteps={per_stage_timesteps}")

        train_fn = make_train_for_stage(stage_cfg, env_params, static_env_params, train_state)
        rng, _rng = jax.random.split(rng)
        # Do not jit the outer call to keep stage changes simple
        out = train_fn(_rng, train_state)
